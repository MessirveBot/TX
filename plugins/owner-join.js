...
let handler = async (m, { conn, text, usedPrefix, command, args, participants, isOwner }) => {
 ...
 if (e.length) await conn.reply(res, ` Hola,mi nombre es TX BOT!

@${owbot} es mi creador,cualquier error le puedes notificar!
fui comprado por: *${m.name}*`, m, {
    mentions: d
     }).then(async () => {
     await delay(7000)
     }).then( async () => {
...
*${conn.user.name}* es uno de los bots multidispositivo de WhatsApp construido con Node.js, *${conn.user.name}* Recién comprado por: *${m.name}*

para ver el Menu del bot escribe

${usedPrefix}help
@${conn.user.jid.split('@')[0]} saldrá automáticamente después de \n\n${msToDate(global.db.data.chats[res].expired - now)}`
  await conn.sendButton(res, mes, igfg, null, [[` Owner`, `${usedPrefix}owner`], [` Menu`, `${usedPrefix}help`]], m, {
        mentions: d
         })
     })
    } catch (e) {
